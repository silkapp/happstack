...
cd happstack-util       && runhaskell Setup configure --user -f tests "$@" && runhaskell Setup build && runhaskell Setup install && \
cd ../happstack-data    && runhaskell Setup configure --user -f tests "$@" && runhaskell Setup build && runhaskell Setup install && \
cd ../happstack-state   && runhaskell Setup configure --user -f tests "$@" && runhaskell Setup build && runhaskell Setup install && \
cd ../ixset             && runhaskell Setup configure --user -f tests "$@" && runhaskell Setup build && runhaskell Setup install && \
